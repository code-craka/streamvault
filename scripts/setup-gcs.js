...
function checkEnvironmentVariables() {
  ...
  if (missing.length > 0) {
    ...
    log('Please check your .env.local file and ensure all GCS variables are set.', 'yellow')
    process.exit(1)
  }
...
function checkServiceAccountFile() {
  ...
  try {
    ...
    if (missingFields.length > 0) {
      log(`❌ Invalid service account file. Missing fields: ${missingFields.join(', ')}`, 'red')
      return false
    }
...
async function runSetup() {
  ...
  try {
    ...
    log('2. Test the storage API: curl http://localhost:3000/api/storage/test', 'cyan')
