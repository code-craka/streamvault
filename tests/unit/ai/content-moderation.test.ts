...
import { ContentModerationService, ModerationResult, ViolationType } from '@/lib/ai/content-moderation'
...
describe('ContentModerationService', () => {
  ...
  describe('moderateText', () => {
    ...
    it('should detect hate speech', async () => {
      ...
      const hateViolation = result.violations.find(v => v.type === 'hate_speech')
      expect(hateViolation).toBeDefined()
      expect(hateViolation?.confidence).toBeGreaterThan(0)
    })

    it('should detect harassment', async () => {
      const harassmentText = 'You are being harassed and bullied by this message'
      
      const result = await moderationService.moderateText(harassmentText, 'chat')
      
      expect(result).toBeDefined()
      expect(result.approved).toBe(false)
      
      const harassmentViolation = result.violations.find(v => v.type === 'harassment')
    ...
    it('should handle different contexts appropriately', async () => {
      ...
      const chatResult = await moderationService.moderateText(borderlineText, 'chat')
      const commentResult = await moderationService.moderateText(borderlineText, 'comment')
      const titleResult = await moderationService.moderateText(borderlineText, 'title')
  ...
  describe('moderateVideo', () => {
    ...
    it('should moderate video metadata', async () => {
      ...
      const result = await moderationService.moderateVideo(videoPath, problematicMetadata)
    ...
    it('should handle video analysis failures gracefully', async () => {
      ...
      const result = await moderationService.moderateVideo(invalidVideoPath, metadata)
  ...
  describe('moderateLiveStream', () => {
    it('should monitor live stream content', async () => {
      ...
      expect(['strict', 'moderate', 'lenient']).toContain(result.moderationLevel)
    })
  ...
  describe('processContentAppeal', () => {
    it('should process appeals with AI assistance', async () => {
      const mockAppeal = {
        ...
        appealReason: 'This was taken out of context and is actually educational content',
      ...
      expect(['approve', 'reject', 'needs_human_review']).toContain(result.recommendation)
      expect(result.confidence).toBeGreaterThanOrEqual(0)
      expect(result.confidence).toBeLessThanOrEqual(1)
    ...
    it('should recommend human review for complex cases', async () => {
      const complexAppeal = {
        ...
        appealReason: 'This is a complex case that requires nuanced understanding',
        status: 'pending' as const,
        submittedAt: new Date(),
      }
  ...
  describe('violation types', () => {
    ...
    violationTypes.forEach(violationType => {
      it(`should detect ${violationType} violations`, async () => {
        ...
        const category = result.categories.find(cat => cat.category === violationType)
