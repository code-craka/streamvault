...
export function ContentManager() {
  ...
  const updateVOD = async (vodId: string, updates: Partial<VODContent>) => {
    try {
      ...
      if (response.ok) {
        setVods(prev =>
          prev.map(vod => (vod.id === vodId ? { ...vod, ...updates } : vod))
        )
        setEditingVod(null)
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <Tabs defaultValue="vods" className="space-y-4">
        ...
        <TabsContent value="vods" className="space-y-4">
                ...
                <Card key={vod.id} className="overflow-hidden">
                  <div className="bg-muted relative aspect-video">
                    ...
                    <div className="absolute left-2 top-2">
                      <Badge
                        variant={
                          vod.status === 'ready' ? 'default' : 'secondary'
                        }
                        className="text-xs"
                      >
                        {vod.status}
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-2">
                      <h4 className="line-clamp-2 font-medium">{vod.title}</h4>
                    ...
                    <div className="mt-4 flex items-center justify-between border-t pt-4">
                      <div className="flex items-center gap-2">
                        <Button
                          onClick={() =>
                            toggleVODVisibility(vod.id, !vod.isPublic)
                          }
                          variant="ghost"
                          size="sm"
                        >
          ...
          {/* Edit VOD Modal */}
          {editingVod && (
            <Card className="fixed inset-0 z-50 m-4 mx-auto my-8 max-h-[90vh] max-w-2xl overflow-y-auto">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setEditingVod({ ...editingVod, title: e.target.value })
                    }
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={value =>
                      setEditingVod({ ...editingVod, category: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    ...
                    onCheckedChange={checked =>
                      setEditingVod({ ...editingVod, isPublic: checked })
                    }
                  />
                ...
                <div className="flex justify-end gap-2 pt-4">
                  <Button variant="outline" onClick={() => setEditingVod(null)}>
                    Cancel
                  </Button>
                  <Button onClick={() => updateVOD(editingVod.id, editingVod)}>
