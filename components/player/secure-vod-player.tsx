...
export function SecureVODPlayer({ 
  ...
  const fetchSignedURL = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/videos/${videoId}/signed-url?tier=${requiredTier}`, {
        headers: {
  ...
  const setupAutoRefresh = useCallback((session: VideoSession) => {
      ...
      <Card className={className}>
        <CardContent className="flex items-center justify-center p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">Please sign in to watch this video</p>
      ...
      <Card className={className}>
        <CardContent className="flex items-center justify-center p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">{playerState.error}</p>
            
            {requiredTier !== 'basic' && (
              <div className="mb-4">
                <Badge className={TIER_COLORS[requiredTier]}>
                  {TIER_ICONS[requiredTier]} {requiredTier.toUpperCase()} Required
                </Badge>
              </div>
            )}
      ...
      <Card className={className}>
        <CardContent className="flex items-center justify-center p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">Unable to load video content</p>
      ...
      {process.env.NODE_ENV === 'development' && playerState.session && (
        <Card className="mt-4">
          ...
          <CardContent className="text-xs space-y-1">
            ...
            <div>Expires: {new Date(playerState.session.expiresAt).toLocaleString()}</div>
            <div>Time Remaining: {getTimeRemaining()}</div>
            <div>Auto-refresh: {refreshIntervalRef.current ? 'Active' : 'Inactive'}</div>
          </CardContent>
        </Card>
      )}
    </div>
