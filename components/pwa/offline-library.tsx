...
import { OfflineManager, type OfflineVideo, type DownloadProgress } from '@/lib/pwa/offline-manager'
import { formatCacheSize } from '@/lib/pwa/pwa-utils'
import { Download, Play, Trash2, Clock, HardDrive, Wifi, WifiOff } from 'lucide-react'
...
export function OfflineLibrary({ className }: OfflineLibraryProps) {
  ...
  const [downloadQueue, setDownloadQueue] = useState<Map<string, DownloadProgress>>(new Map())
  ...
  const downloadVideo = async (videoId: string, quality: '480p' | '720p' | '1080p' | '4K' = '720p') => {
    if (!user) return
    ...
    if (!offlineManager.canDownload(subscriptionTier, currentDownloads)) {
      ...
      alert(`Download limit reached. Your ${subscriptionTier} plan allows ${limit === -1 ? 'unlimited' : limit} downloads.`)
      return
    }
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {downloadQueue.size > 0 && (
        <Card className="p-4">
          <h3 className="font-medium mb-3">Downloads in Progress</h3>
      ...
      {offlineVideos.length === 0 ? (
        <Card className="p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">
            Download videos to watch them offline. Premium and Pro users can download videos for offline viewing.
          </p>
          <Button
          ...
          {offlineVideos.map((video) => (
            <Card key={video.videoId} className="overflow-hidden">
              ...
              <div className="p-4">
                ...
                {video.creatorName && (
                  <p className="text-sm text-gray-600 mb-2">{video.creatorName}</p>
                )}
