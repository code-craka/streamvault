...
import { runFirebaseTests, validateFirebaseEnvironment } from '@/lib/firebase/connection-test'
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (testType) {
      case 'client':
        const { testClientConnection } = await import('@/lib/firebase/connection-test')
        result = await testClientConnection()
        break
      case 'admin':
        const { testAdminConnection } = await import('@/lib/firebase/connection-test')
        result = await testAdminConnection()
        break
      case 'crud':
        const { testCRUDOperations } = await import('@/lib/firebase/connection-test')
