  ...
  try {
    ...
    streamsSnapshot.docs.forEach(doc => {
      ...
      if (stream.endedAt && stream.startedAt) {
        const duration = (stream.endedAt.toDate().getTime() - stream.startedAt.toDate().getTime()) / (1000 * 60)
    ...
    const overages = {
      storage: Math.max(0, storageUsed - 50) * 0.10, // $0.10 per GB over limit
    ...
    const overageTotal = overages.storage + overages.bandwidth + overages.apiCalls
