...
export async function GET(req: NextRequest) {
  try {
    ...
    if (validatedParams.resourceType)
      filters.resourceType = validatedParams.resourceType
    if (validatedParams.startTime)
      filters.startTime = new Date(validatedParams.startTime).getTime()
    if (validatedParams.endTime)
      filters.endTime = new Date(validatedParams.endTime).getTime()
...
export async function POST(req: NextRequest) {
  try {
    ...
    const auditId = await auditTrailService.logEvent({
      ...
      changes:
        validatedEntry.oldValues && validatedEntry.newValues
          ? generateChanges(validatedEntry.oldValues, validatedEntry.newValues)
          : undefined,
      ...
      category: determineCategory(
        validatedEntry.action,
        validatedEntry.resourceType
      ),
...
function determineCategory(action: string, resourceType: string): string {
  ...
  if (resourceType === 'stream' || resourceType === 'video')
...
function isComplianceRelevant(action: string, resourceType: string): boolean {
  ...
  return (
    complianceActions.some(a => action.includes(a)) ||
