...
class FraudDetectionEngine {
  ...
  async analyzeTransaction(
    ...
    try {
      ...
      if (eventData.location && userProfile) {
        const geoSignals = this.analyzeGeolocation(
          eventData.location,
          userProfile
        )
      ...
      if (userProfile) {
        const deviceSignals = this.analyzeDevice(
      ...
      return {
        ...
        reasoning: [
          'Fraud detection system encountered an error - manual review required',
        ],
      }
    }
  }

  private async analyzeVelocity(
    userId: string,
    eventType: string
  ): Promise<FraudSignal[]> {
    const signals: FraudSignal[] = []
  ...
  private analyzeGeolocation(
    ...
    const isCommonLocation = userProfile.commonLocations.some(
      location =>
  ...
  private analyzeBehavior(
    ...
    const isTypicalTime = userProfile.typicalLoginTimes.some(
  ...
  private async analyzePaymentFraud(
    ...
    try {
      ...
      if (
        userProfile &&
        amount > userProfile.paymentPatterns.averageAmount * 5
      ) {
        signals.push({
          type: 'payment',
          severity: 'high',
  ...
  private generateReasoning(
    ...
    if (signals.length === 0) {
      ...
      signals.forEach(signal => {
        reasoning.push(
          `- ${signal.description} (${signal.severity} risk, ${(signal.confidence * 100).toFixed(1)}% confidence)`
        )
      })
    }
  ...
  async getFraudEvents(
    ...
    try {
      ...
      if (filters.startTime) {
        q = query(
          q,
          where('timestamp', '>=', Timestamp.fromMillis(filters.startTime))
        )
      }
      if (filters.endTime) {
        q = query(
          q,
          where('timestamp', '<=', Timestamp.fromMillis(filters.endTime))
        )
      }
